...
function guessed_velocity_equation_solver(
    ...
    CATHARE = @. γ*((ρ0G.e*α0G.e*α0L.e*ρ0L.e)/(ρ0L.e*α0G.e + ρ0G.e*α0L.e))*(u0G.e - u0L.e)^2
...
function continuity_equation_solver(
    αx_G :: Vector{Float64},
    ρx_G :: Vector{Float64},
    ux_G :: Vector{Float64},
    αx_L :: Vector{Float64},
    ...
    αxG = AlphaMainGrid(αx_G, ux_G, N)
    ρxG = RhoMainGrid(ρx_G, ux_G, N)

    ρG = RhoMainGrid(ρ_G, ux_G, N)
    
    αxL = AlphaMainGrid(αx_L, ux_L, N)
    ρxL = RhoMainGrid(ρx_L, ux_L, N)
    
    ρL = RhoMainGrid(ρ_L, ux_L, N)

    α_G = void_fraction_muscl(αx_G, ρx_G, ux_G, αxG, ρxG, ρG, N, Δx, Δt)
    α_L = void_fraction_muscl(αx_L, ρx_L, ux_L, αxL, ρxL, ρL, N, Δx, Δt)
